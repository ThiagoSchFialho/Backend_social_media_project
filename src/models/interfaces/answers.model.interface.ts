...
export interface IAnswersModel {
    createAnswer(text: string, dateTime: string, user_id: number, comment_id?: number, parent_answer_id?: number): Promise<Answers>;
    getByComment(comment_id: number): Promise<Answers[]>;
    getByAnswer(parent_answer_id: number): Promise<Answers[]>;
    updateAnswer(id: number, text: string, dateTime: string): Promise<Answers>;
